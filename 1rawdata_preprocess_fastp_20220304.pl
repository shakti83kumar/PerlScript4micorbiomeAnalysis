...
##
##
my $inFolder       = '/home/thsti/metagenomicsAnalysis/01BDasSir/20220518_R40'; #folder path of raw reads
...
foreach my $eachFile (sort (keys %baseNameHash))
 {
	...
	my $inRevsdRead = $inFolderNamePath.$eachFile.'_L001_R2_001.'.$rawreadfmt;
	...
	&FastpProcess($fastp, $inFarwdRead, $inRevsdRead, $outFarwdRead, $outRevsdRead, $unpaired_outFarwdRead, $unpaired_outRevsdRead, $merged_reads, $failed_out_reads, $outputBase);
...
sub FastpProcess
	{
		my ($prg, $iR1, $iR2, $oR1, $oR2, $unR1, $unR2, $mgR, $flR, $out) = @_;
